...
from aurora.core.exceptions import DataFetchError, RateLimitError
...
class BaseAPIClient(ABC):
    ...
    async def _request(
        ...
        Args:
            ...
            cache_key_parts: Tuple of (endpoint_name, identifier, date) for caching
        ...
        # Check cache first
        if cache_key_parts:
            endpoint_name, identifier, trade_date = cache_key_parts
            cached = self.cache.load_json(
                self.SOURCE_NAME, endpoint_name, identifier, trade_date
            )
            if cached is not None:
                logger.debug(
                    f"Cache hit: {self.SOURCE_NAME}/{endpoint_name}/{identifier}/{trade_date}"
                )
                return cached
    ...
    async def health_check(self) -> bool:
        ...
        """
        try:
            # Subclasses should override with appropriate endpoint
            return True
        except Exception:
            return False
